...
with col3:
    order = tuple(cfg.get("ml", {}).get("arima_order", [1,1,1]))
...
if prices is not None:
    ...
    show_df = pd.concat([prices.rename(columns={"close":"value"})[["date","value"]].assign(kind="history"),
                         fc_df.rename(columns={"forecast":"value"}).assign(kind="forecast")], ignore_index=True)
